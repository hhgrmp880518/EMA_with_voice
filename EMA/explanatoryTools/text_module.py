...
def text_add_title(args):
    title = f'我們來用數線計算{args[0]}+{args[1]}的答案'
    end = f'所以{args[0]}+{args[1]}的答案是{args[0]+args[1]}'
    return title, end
...
def text_sub_title(args):
    title = f'我們來用數線計算{args[0]}減{args[1]}的答案'
    end = f'所以{args[0]}減{args[1]}的答案是{args[0]-args[1]}'
    return title, end
...
def text_mul_title(args):
    end = f"也就是{args[0]}成{args[1]}的結果"

    title = f"計算{args[0]}成{args[1]}，也就是會有{args[1]}個{args[0]}加起來，我們來把{args[0]}的數線畫下來"
...
def text_mul_line(args, nums):
    ...
    '''
    # text_group 1, title
    title = f"計算{problems[-1]}"

    # 做乘法的數字超過2個，才會有「先看」的行為
    if len(args) > 2:
        title += f"，我們先看{problems[0]}"

    title += f"，也就是會有{args[1]}個{args[0]}加起來，我們先把{args[0]}的數線畫下來"
