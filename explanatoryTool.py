    ...
    def numberLineAdd(
        ...
        **kwargs
        ...
        if display:
            ...
            from text_module import text_add_line
            text_group = text_add_line(args, ans)

            # 根據加法數字的動畫數量分配語音
            for i in range(len(line_group)):
                with self.voiceover(text=text_group[i]):
                        self.play(Write(line_group[i]), run_time=1/speed)
            # 答案搭配最後一段語音
            with self.voiceover(text=text_group[-1]):
                    self.play(Write(sum_group), run_time=1/speed)
    ...
    def numberLineSub(
        ...
        **kwargs
        ...
        if display:
            ...
            from text_module import text_sub_line
            text_group = text_sub_line(args, ans)

            # 被減數的語音
            with self.voiceover(text=text_group[0]):
                    self.play(Write(minuend_group), run_time=1/speed)
            # 減數及結果的語音                    
            for i in range(len(line_group)):
                    with self.voiceover(text=text_group[i+1]):
                            self.play(Write(line_group[i]), run_time=1/speed)
        ...
        if display:
            ...
            for i in range(len(line_groups)):
                ...
                for j in range(len(line_groups[i])):
                    ...
                    if i != 0 and j == 0:
                        with self.voiceover(text=text_group[k]):
                                self.play(Create(line_groups[i][j]), run_time=1/speed)
                                k+=1
                    else:
            ...
            with self.voiceover(text=text_group[k]):
                self.play(Write(product_brace), Write(product_label), run_time=1/speed)
